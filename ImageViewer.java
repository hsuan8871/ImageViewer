...
public class ImageViewer {
	...
	private ImageViewer(String input_filename) {
	...
	public void setPixel(int x, int y, Color colour){
		...
		if (x < 0 || x >= width)
				throw new Error(String.format("Invalid x coordinate"));
		if (y < 0 || y >= height)
			throw new Error(String.format("Invalid y coordinate"));
	...
	private void runAlgorithm(int X, int Y){
		int width = currentImage.getWidth();
		int height = currentImage.getHeight();
	...
	private void loadImage(){
      ...
      System.out.println("Unable to open the image file.");
      System.exit(1);
	...
	private void initialize() {
		...
		final String[] algorithmList = {"Flood Fill with DFS", "Flood Fill with BFS", "Outline Region with DFS", "Outline Region with BFS"};
	...
	class AnimateHelper{
		...
		AnimateHelper(int w, int h){
			...
			pixelsPerMillisecond = ((totalPixels*0.01)/10000);
		}
	...
	public static void spawn(final String input_filename) {
		java.awt.EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					ImageViewer window = new ImageViewer(input_filename);
	...
	public static void main(String[] args){
		if (args.length < 1){
			System.out.printf("Usage: ImageViewer <input image>\n");
			return;
		}
		...
		if (!input_filename.toLowerCase().endsWith(".png")){
      	System.out.println("Input file must be a PNG image.\n");
        System.exit(0);
    }
